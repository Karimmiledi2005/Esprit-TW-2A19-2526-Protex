...
<head>
    ...
    <style>
        ...
        .message.agent { align-self: flex-end; align-items: flex-end; }
        .message.client { align-self: flex-start; align-items: flex-start; }
        .bubble { padding: 10px 15px; border-radius: 15px; font-size: 14px; position: relative; }
        .message.agent .bubble { background: #007bff; color: white; border-bottom-right-radius: 2px; }
        .message.client .bubble { background: #e9ecef; color: black; border-bottom-left-radius: 2px; }
...
<div class="input-area">
    ...
    <button class="btn btn-primary rounded-circle" style="width:45px; height:45px;" onclick="sendMessage()">
    ...
    try {
        ...
        if (json.success) {
            chatContainer.innerHTML = json.data.map(m => {
                const isAgent = m.role_sender !== 'client';
                const cls = isAgent ? 'agent' : 'client';
                const name = isAgent ? 'Vous (Agent)' : 'Client';
                return `
                <div class="message ${cls}" style="margin-left: ${isAgent ? 'auto' : '0'}; margin-right: ${isAgent ? '0' : 'auto'};">
