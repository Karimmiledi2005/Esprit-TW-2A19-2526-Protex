...
<head>
    <meta charset="utf-8">
    <title>Rembourser un paiement — Protex Admin</title>
    ...
    <style>
        .page-hero {
            ...
            border-radius: 22px;
            background:
                radial-gradient(circle at top right, rgba(0,180,216,.16), transparent 35%),
...
<?php include __DIR__ . '/../assets/includes/sidebar.php'; ?>
    ...
    <main class="main">
        <div class="topbar">
            <div>
                <div class="topbar-title">Remboursement du paiement</div>
            ...
            <?php else: ?>
                ...
                <section class="page-hero">
                    <div class="page-hero-head">
                        <div>
                            <h1 class="page-hero-title">Confirmer le remboursement</h1>
                            <p class="page-hero-sub">
                                Cette action marquera ce paiement comme <strong style="color:#7dd3fc;">remboursé</strong>.
                                Merci de préciser le motif du remboursement pour conserver un historique financier clair.
                            </p>
                        </div>
                ...
                <div class="confirm-grid">
                    ...
                    <div class="detail-card">
                        <div class="detail-card-head">
                            <div class="detail-card-title">Motif du remboursement</div>
                        </div>
                            ...
                            <div class="side-stack">
                                <div class="confirm-box">
                                    ...
                                    <div class="confirm-box-text">
                                        Le remboursement doit être justifié. Le motif sera conservé dans l’historique
                                        pour tracer clairement la raison de l’annulation financière.
                                    </div>
                                </div>
                                ...
                                <div class="mini-info">
                                    ...
                                    <div class="mini-info-sub">Le paiement doit être validé pour être remboursé.</div>
                                </div>

                                <div class="mini-info">
                                    <div class="mini-info-title">Formulaire</div>

                                    <?php if ($statut !== 'valide'): ?>
                                        <div class="mini-info-sub">
                                            Ce paiement n’est pas validé. Il ne peut pas être remboursé depuis cette page.
                                        </div>
                                    ...
                                    <?php else: ?>
                                        <form method="post" action="<?= e($BASE_URL) ?>/controller/PaiementController.php?action=rembourser&id=<?= (int)($paiement['id_paiement'] ?? 0) ?>" style="margin:0;" onsubmit="return verifierMotifRemboursement();">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div style="display:flex; gap:10px;">
                                                    <label style="display:flex; align-items:center; gap:5px; color:#fff; cursor:pointer;">
                                                        <input type="radio" name="type_remboursement" value="total" checked onchange="toggleMontantPartial(false)"> Total
                                                    </label>
                                                    <label style="display:flex; align-items:center; gap:5px; color:#fff; cursor:pointer;">
                                                        <input type="radio" name="type_remboursement" value="partiel" onchange="toggleMontantPartial(true)"> Partiel
                                                    </label>
                                                </div>
                                            </div>

                                            <div class="form-group" id="div-montant-partiel" style="display:none;">
                                                <label for="montant_partiel" class="form-label">Montant à rembourser (Max: <?= formatMoney($paiement['montant']) ?>) *</label>
                                                <input type="number" id="montant_partiel" name="montant_partiel" step="0.001" min="0" max="<?= (float)$paiement['montant'] ?>" style="width:100%; height:40px; padding:0 14px; border-radius:14px; border:1px solid rgba(255,255,255,0.1); background:rgba(255,255,255,0.05); color:#fff; outline:none;">
                                            </div>

                                            <div class="form-group">
                                                <label for="motif" class="form-label">Motif du remboursement *</label>
                                                <textarea
                                                    id="motif"
                                                    name="motif"
                                                    class="form-textarea"
                                                    placeholder="Exemple : erreur de facturation, demande client validée, transaction annulée..."
                                                    required
                                                ><?= e($_POST['motif'] ?? '') ?></textarea>
                                                <div class="form-help">
                                                    Indique un motif professionnel, clair et exploitable par l’administration.
                                                </div>
                                            </div>

                                            <div class="action-bar">
                                                <button type="submit" class="btnx refund" onclick="return confirm('Confirmer le remboursement de ce paiement ?')">
                                ...
                                <div class="mini-info">
                                    ...
                                    <div class="mini-info-sub">
                                        Utilise un motif précis comme :
                                        <em>demande client acceptée</em>,
                                        <em>double paiement détecté</em> ou
                                        <em>erreur de traitement financier</em>.
                                    </div>
                                </div>
                            </div>
                        </div>
...
function toggleMontantPartial(show) {
    document.getElementById('div-montant-partiel').style.display = show ? 'block' : 'none';
}

function verifierMotifRemboursement() {
    const textarea = document.getElementById('motif');
    if (!textarea) return true;

    const value = textarea.value.trim();
    if (value.length < 5) {
        alert('Merci de saisir un motif de remboursement plus précis.');
        textarea.focus();
        return false;
    }
    
    const isPartial = document.querySelector('input[name="type_remboursement"][value="partiel"]').checked;
    if (isPartial) {
        const montantInput = document.getElementById('montant_partiel');
        const montant = parseFloat(montantInput.value);
        if (isNaN(montant) || montant <= 0 || montant > <?= (float)$paiement['montant'] ?>) {
            alert('Veuillez saisir un montant partiel valide.');
            montantInput.focus();
            return false;
        }
    }
