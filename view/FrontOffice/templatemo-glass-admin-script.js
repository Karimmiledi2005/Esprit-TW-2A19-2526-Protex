...
function initPasswordStrength() {
    ...
    if (!checklist.minchar || !checklist.upper || !checklist.lower || !checklist.number || !checklist.symbol) {
        return;
    }
    ...
    function updateStrength(password) {
        ...
        // Reset
        Object.values(checklist).forEach(item => item.classList.remove('valid'));

        if (password.length >= 8) {
            checklist.minchar.classList.add('valid');
            score++;
        }
        if (/[A-Z]/.test(password)) { checklist.upper.classList.add('valid'); score++; }
        if (/[a-z]/.test(password)) { checklist.lower.classList.add('valid'); score++; }
        if (/\d/.test(password))    { checklist.number.classList.add('valid'); score++; }
        if (/[^A-Za-z0-9]/.test(password)) { checklist.symbol.classList.add('valid'); score++; }
    ...
    passwordInput.addEventListener('input', () => {
        ...
        // Check confirmation match if available
        if (confirmInput && confirmError && confirmInput.value) {
